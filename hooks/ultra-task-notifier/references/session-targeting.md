...
- **VS Code return reveals before activating.** macOS decides which Space to switch to at *activation* time, from the app's then-frontmost window — and a window focused after activation does not pull a second Space switch. A single `open -b <vscode> <ws>` therefore lands on whichever VS Code window is nearest when windows sit on several Spaces. The click handler instead opens the workspace with `-g` (no activation) so VS Code raises the owning window first, then activates the app a beat later, making the Space switch target that window.
