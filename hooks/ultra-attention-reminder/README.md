...
掛在 `Stop`（回合結束）與 `Notification`（等待權限或輸入）兩個事件上的純 side-effect hook。只有在「你沒在看正在跑的那個 session」時才發送：iTerm 可精準到 session，其餘終端機退回 App 級。點擊通知會跳回 Claude 所在的視窗。通知後端採漸進式增強（Progressive Enhancement）：優先使用 `Reminder.app`（自訂圖示），未建置時退回 `osascript`。
...
- **手動安裝**：
  ...
  - 跑 `app/build.sh` 自行建置 `Reminder.app`（細節見下方 Reminder.app）
  - 依下方「註冊」手動登記
- **執行腳本**：
  ...
  - 連結進 `~/.claude/hooks/`；本 hook 帶 `install.sh`，連結後自動建置 `Reminder.app`，並**檢查（不改寫）**註冊狀態
  - 註冊仍需你手動完成（見下方）
- **註冊到 `settings.json`（手動，兩種安裝方式都需要）**：
  ...
  4. 完整 JSON 見 repo 的 `settings.hooks.json`，照抄／合併進去後存檔即生效
...
- **通知方式採漸進式增強**：
  ...
  - 若尚未建置 `Reminder.app`，則自動退回使用 `osascript`
...
閘門與點擊跳回共用同一個想法：**在發通知的當下記住「Claude Code 在哪個 session」，這個身分用兩次**——判斷該不該通知，以及點擊時跳回哪裡。點擊處理跑在 `Reminder.app` 裡，拿不到原 session 的環境，所以由 hook 在發通知時把身分寫進 `--args`。
...
- **iTerm 用 AppleScript `select`**：
  ...
  - 閘門查詢則在 hook（iTerm 子孫程序）內執行，免授權
- **跨桌面 / 螢幕交給系統處理**：
...
- **首次建置**：
  - 首次安裝時由 `install.sh` 自動處理
  - 若 `Reminder.app` 尚未存在，會建置到 `~/.claude/tools/Reminder.app`
- **重新建置**：
...
- **使用方式**：
  - 建置完成後由 hook 自動呼叫，使用者不需要手動開啟 `Reminder.app`
- **無第三方相依**：
